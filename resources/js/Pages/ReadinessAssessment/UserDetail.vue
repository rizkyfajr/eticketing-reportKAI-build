...
const changeDate = (date) => {
  ...
  Inertia.get('/readiness-assessment/user/' + props.user.id, { date: date }, {
    preserveState: true,
    preserveScroll: true,
  });
};
...
const goBackToMonitoring = () => {
  Inertia.get('/readiness-assessment/admin-monitoring', { date: selectedDate.value });
};
...
const answeredCount = computed(() => {
  ...
  Object.values(props.groupedData).forEach(questions => {
    questions.forEach(q => {
      if (q.jawaban !== 'Belum Diisi') count++;
    });
  });
  return count;
});
...
<template>
  <DashboardLayout title="Detail Assessment User">
    <Head :title="`Detail Assessment - ${user.name}`" />
    ...
    <div class="py-6">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
          ...
          <div class="p-6">
            <div class="flex items-center justify-between">
              <div class="flex items-center">
                <button
                  @click="goBackToMonitoring"
                  class="mr-4 p-2 rounded-md hover:bg-gray-100 transition"
                >
                ...
                <div>
                  <h2 class="text-2xl font-bold text-gray-800">
                    Detail Assessment User
                  </h2>
                  <p class="text-gray-600 mt-1">
                    <span class="font-semibold">[{{ user.username }}] {{ user.name }}</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  <p v-if="filledDates.length === 0" class="text-gray-500 text-sm italic">
                    Belum ada data
                  </p>
                </div>
              </div>
        ...
        <!-- Data Assessment -->
        <div v-if="hasAssessment" class="bg-white shadow-sm sm:rounded-lg">
          <div class="p-6">
        ...
        <div v-else class="bg-white shadow-sm sm:rounded-lg">
          <div class="p-12 text-center">
            ...
            <p class="mt-2 text-gray-600">
              User ini belum mengisi assessment untuk tanggal {{ formatDate(selectedDate) }}
            </p>
          </div>
        </div>
      </div>
