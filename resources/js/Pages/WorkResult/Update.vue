...
const submit = () => {
  ...
  form.patch(route('work-results.update', props.result.id), {
    onSuccess: () => {
      Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: 'Laporan kerja berhasil diubah.',
        timer: 1500,
...
<template>
  <DashboardLayout :title="__('Laporan Pekerjaan')">
    <div class="transition-all duration-300">
      <main class="p-4">
        <h2 class="font-bold text-lg mb-1">Laporan Pekerjaant</h2>
        <p class="text-sm text-gray-500 mb-4">Halaman Update Data Laporan Pekerjaan</p>
            ...
            <form @submit.prevent="submit" class="grid grid-cols-2 gap-6 p-4">
              ...
              <div class="col-span-2 flex justify-end mt-4 gap-2">
                ...
                <button class="bg-gray-600 text-white px-4 py-2 rounded-md hover:bg-gray-700 mt-10">
                    <Link :href="route('work-results.index')">
                        <Icon name="edit" />
                        <p>{{ __('Kembali') }}</p>
                    </Link>
                </button>
